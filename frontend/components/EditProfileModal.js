...
const EditProfileModal = ({ visible, onClose }) => {
    ...
    const uploadImage = async (uri) => {
        ...
        formData.append('file', {
            uri,
            name: `${Date.now()}_${uri.split('/').pop()}`,
            type: 'image/jpeg'
        });

        try {
            const response = await axios.post(`http://${ip}:${port}/user/upload-image`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });

            return response.data; // Returnează numele fișierului
        } catch (error) {
    ...
    const pickImage = async () => {
        ...
        if (!result.canceled) {
            ...
            if (fileName) {
                setProfileData({ ...profileData, profileImage: fileName });
            }
        }
    };
