...
const EventDetailsModal = ({ event, visible, onClose, onUpdateEvent }) => {
  ...
  const [isEventFeedbackModalVisible, setIsEventFeedbackModalVisible] = useState(false); // Stare pentru vizibilitatea feedback modal
                ...
                {eventData.registeredPlayers.map((player, index) => (
                  <View style={styles.playerItem} key={player.id}>
                    ...
                    <Text style={styles.playerName}
                      onPress={() => {
                        if (!player.username.includes('Adaugat de')) {
                          handleOpenProfileModal(player.id, player.username);
                        }
                      }}
                    >{player.username ? player.username.replace(/Adaugat de \d+/, '').trim() : ''}
                    </Text>
                    {player.username && player.username.includes(`Adaugat de ${userData.id}`) && (
